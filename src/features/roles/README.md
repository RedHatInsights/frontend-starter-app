...
> **This is a REFERENCE IMPLEMENTATION** showing how to build a feature using the starter app's infrastructure. When creating a new app from this template, replace this folder with your own features.

The roles example demonstrates: feature island architecture, ServiceContext DI usage, TanStack Query with query keys factory, MSW handler factories for Storybook, and user journey testing with play functions.

It implements a full CRUD workflow using real `@redhat-cloud-services/rbac-client` types. Showcases TanStack Query hooks, ServiceContext DI, TableView with useTableState, wizard-in-modal for create/edit, MSW handler factories, and user journey Storybook stories.
